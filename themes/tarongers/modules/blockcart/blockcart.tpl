...
-->
		...
		{/if}
		<p id="cart-buttons">
	<!--		{if $order_process == 'order'}<a href="{$link->getPageLink("$order_process.php", true)}" class="button_mini" title="{l s='Cart' mod='blockcart'}">{l s='Cart' mod='blockcart'}</a>{/if}  -->
			<a href="{$link->getPageLink("$order_process.php", true)}{if $order_process == 'order'}?step=0{/if}" id="button_order_cart" class="button_small" title="{l s='Check out' mod='blockcart'}">{l s='Check out' mod='blockcart'}</a>
		</p>
	</div>
	</div>
</div>
