...
class piscesslider extends Module
{
	/** @var max image size */
	...
	protected $_xml;
 	...
 	{
 	 	$this->name = 'piscesslider';
		$this->tab = 'front_office_features';
 	 	$this->version = '1.0';
		/* The parent construct is required for translations */
	 	parent::__construct();
		$this->page = basename(__FILE__, '.php');
	 	$this->displayName = $this->l('JQuery Pisces Slider');
 	 	...
 	 	if (isset($_POST['submitUpdate']))
 	 	{
			...
			foreach ($_POST['link'] as $link)
			{
				...
				elseif (!$tmpName = tempnam(_PS_TMP_IMG_DIR_, 'PS') OR !move_uploaded_file($_FILES['link_'.$i.'_img']['tmp_name'], $tmpName))
					return false;
                elseif (!imageResize($tmpName, dirname(__FILE__).'/slides/slide_0'.$i.'.png'))
					$this->_html .= $this->displayError($this->l('An error occurred during the image upload.'));
                unlink($tmpName);
				}
				if ($line = $this->putContent($newXml, 'img', 'slides/slide_0'.$i.'.png'))
					$newXml .= $line;
				$newXml .= "\n".'	</link>'."\n";
				$i++;
			}
	...
	public function _getFormItem($i, $last)
	{
		...
		$output .= '
				...
				<div class="margin-form">
					<div><img src="'.$this->_path.'slides/slide_0'.$i.'.png" alt="" title="" style="width:250px; height:auto;" /></div>
