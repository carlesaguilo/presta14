...
class Blocktext extends Module
{
 	function __construct()
 	{
		...
		$this->displayName = $this->l('Modulo texto');
		$this->description = $this->l('Modulo de información con texto ');
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('submitBlockText'))
		{
			$errors = array();
	...
	{					
		...
		<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post">
			<fieldset><legend><img src="'.$this->_path.'logo.gif" alt="" title="" />'.$this->l('Settings').'</legend>
				...
				<center><input type="submit" name="submitBlockText" value="'.$this->l('Save').'" class="button" /></center>
			</fieldset>
		</form>';
		return $output;
	}
	...
	function hookLeftColumn($params)
	{
		...
		if ($url && ($contents = @file_get_contents($url)))
			...
			{
				Tools::dieOrLog(Tools::displayError('Error: invalid RSS feed in blocktext module').' '.$e->getMessage(), false);
			}
